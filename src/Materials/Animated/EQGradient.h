...
private:
    ...
    RGBColor rainbowSpectrum[7] = {RGBColor(255, 0, 0), RGBColor(255, 100, 0), RGBColor(60, 255, 0), RGBColor(0, 255, 0), RGBColor(0, 255, 100), RGBColor(0, 50, 255), RGBColor(50, 0, 255)};
    GradientMaterial<7> gM = GradientMaterial<7>(rainbowSpectrum, 1.0f, false);
    ...
    RGBColor GetRGB(Vector3D position, Vector3D normal, Vector3D uvw) override {
        ...
        if (yColor <= height){
            if(x <= size.X && x >= 2 * (size.X) / 3){
                return material->GetRGB(Vector3D(1.0f - height - yColor, 0, 0), Vector3D(), Vector3D()).HueShift(180);
            }
            else if(x <= 2 * (size.X) / 3 && x >= (size.X) / 4){
                return material->GetRGB(Vector3D(1.0f - height - yColor, 0, 0), Vector3D(), Vector3D()).HueShift(120);
            }
            else{
                return material->GetRGB(Vector3D(1.0f - height - yColor, 0, 0), Vector3D(), Vector3D()).HueShift(60);
            }
        }
        else{
