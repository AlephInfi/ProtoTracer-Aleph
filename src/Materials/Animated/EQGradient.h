...
private:
    ...
    RGBColor Spectrum[5] = {RGBColor(255, 0, 0), RGBColor(255, 150, 0), RGBColor(0, 255, 0), RGBColor(0, 255, 100), RGBColor(0, 50, 255)};
    GradientMaterial<5> gM = GradientMaterial<5>(Spectrum, 1.0f, false);
    ...
    EQ(Vector2D size, Vector2D offset, bool bounce = false, bool flipY = false, bool mirrorY = false){
        ...
        if (bounce){
            for (uint8_t i = 0; i < 128; i++){
                bPhy[i] = new BouncePhysics(15.0f, 5.0f);
            }
        }
    }
