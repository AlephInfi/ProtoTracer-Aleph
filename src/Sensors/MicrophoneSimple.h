...
public:
    ...
    float Update(){
        float read = analogRead(pin) * 2.0f / 9.0f;
        ...
        float normalized = Mathematics::Constrain(amplitude - minimum - 250.0f, 0.0f, 4000.0f);
        ...
        Serial.print('\t');
        Serial.println(truncate * 10.0f);
        Serial.println(Mathematics::Constrain(truncate, 0.0f, 1.0f));
        */
